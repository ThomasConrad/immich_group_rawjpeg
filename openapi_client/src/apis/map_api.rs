...
pub async fn reverse_geocode(configuration: &configuration::Configuration, lat: f64, lon: f64) -> Result<Vec<models::MapReverseGeocodeResponseDto>, Error<ReverseGeocodeError>> {
    ...
    let local_var_uri_str = format!("{}/map/reverse-geocode", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ReverseGeocodeError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
