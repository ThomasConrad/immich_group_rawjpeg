...
use thaw::{Button, ButtonVariant, Input};
use thiserror::Error;
use tracing::info;
use uuid::Uuid;
...
#[server(GetGroups, "/groups")]
pub async fn get_groups() -> Result<Vec<Vec<Uuid>>, ServerFnError> {
    info!("Fetching groups");
    Ok(vec![vec![Uuid::new_v4(), Uuid::new_v4()]])
}
